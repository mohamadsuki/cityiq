...
create policy if not exists "City settings readable by everyone"
  on public.city_settings for select
  using (true);

create policy if not exists "Mayor/Ceo can insert city settings"
  on public.city_settings for insert
  with check (public.has_role(auth.uid(), 'mayor') or public.has_role(auth.uid(), 'ceo'));

create policy if not exists "Mayor/Ceo can update city settings"
...
create policy if not exists "Branding public read"
  on storage.objects for select
  using (bucket_id = 'branding');

create policy if not exists "Branding insert by mayor/ceo"
  on storage.objects for insert
  with check (bucket_id = 'branding' and (public.has_role(auth.uid(), 'mayor') or public.has_role(auth.uid(), 'ceo')));

create policy if not exists "Branding update by mayor/ceo"
  on storage.objects for update
  using (bucket_id = 'branding' and (public.has_role(auth.uid(), 'mayor') or public.has_role(auth.uid(), 'ceo')));

create policy if not exists "Branding delete by mayor/ceo"
