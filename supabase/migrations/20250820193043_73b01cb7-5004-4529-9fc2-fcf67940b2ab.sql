-- Drop all demo-related policies across ALL tables first
...
-- Drop all existing policies on projects table first (these reference demo functions)
...
DROP FUNCTION IF EXISTS public.has_role_demo(uuid, app_role) CASCADE;
DROP FUNCTION IF EXISTS public.has_department_demo(uuid, department_slug) CASCADE;
